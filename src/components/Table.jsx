...
export const Table = ({ products, isWon, isAdmin, handleDeleteProduct, handleSellProduct }) => {
  return (
    <>
          ...
          {products?.map((product, index) => (
            ...
            <tr className="bg-white border-b hover:bg-gray-50" key={index}>
              ...
              <td className="px-6 py-4">
                <img className="w-10 h-10 rounded-md" src={product?.images && product.images[0]?.filePath || product?.image?.filePath } alt={product?.title || "Product image"} />
              </td>
              {!isWon && (
                  <>
                    <td className="px-6 py-4">
            ...
            {!isAdmin && (
              ...
              <td className="px-6 py-4 text-center flex items-center gap-3 mt-1">
               ...
               {isAdmin ? (
                        <NavLink to={`/product/admin/update/${product?._id}`} type="button" className="font-medium text-green">
                          <CiEdit size={25} />
                        </NavLink>
                      ) : (
