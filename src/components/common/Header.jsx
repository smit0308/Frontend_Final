...
import { getuserProfile, selectIsLoggedIn, selectUser } from "../../redux/features/authSlice";
...
export const Header = () => {
  ...
  return (
    <>
      <header className={isHomePage ? `header py-1 style={{ backgroundColor: "#14334f" }} ${isScrolled ? "scrolled" : ""}` : `header bg-white shadow-s1 ${isScrolled ? "scrolled" : ""}`}>
        <Container>
          <nav className="p-4 flex justify-between items-center relative">
            ...
            <div className="flex items-center gap-8 icons">
              <div className="hidden lg:flex lg:items-center lg:gap-8">
                ...
                <CustomNavLink href="/register" className={`${!isHomePage || isScrolled ? "bg-[#091c2e] text-white" : "bg-white text-primary"} px-8 py-2 rounded-full shadow-md`}>
                  Join
                </CustomNavLink>
                </ShowOnLogout>
                <ShowOnLogin>
